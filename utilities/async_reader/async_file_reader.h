...
#include "utilities/async_reader/common.h"
#include "utilities/async_reader/concurrent_set.h"
#include "utilities/async_reader/disk_io.h"
...
struct CallbackOverLapped : public OVERLAPPED {
  ...
  CallbackOverLapped(PrioritizedDiskFileReaderResource* p_registeredResource)
      : c_registeredResource(p_registeredResource), m_data(nullptr) {}
};

struct PrioritizedDiskFileReaderResource {
  CallbackOverLapped m_col;

  PrioritizedDiskFileReaderResource() : m_col(this) {}
};
}  // namespace DiskUtils
...
class HandleWrapper {
 public:
  ...
  HandleWrapper(HandleWrapper&& p_right)
      : m_handle(std::move(p_right.m_handle)) {}
 ...
 private:
  typedef std::unique_ptr<typename std::remove_pointer<HANDLE>::type,
                          HandleDeleter>
      UniqueHandle;
...
class RequestQueue {
 public:
  ...
  void push(AsyncReadRequest* j) {
    ::PostQueuedCompletionStatus(m_handle.GetHandle(), 0, NULL,
  ...
  bool pop(AsyncReadRequest*& j) {
    ...
    ULONG_PTR key;
    OVERLAPPED* ol;
    BOOL ret = ::GetQueuedCompletionStatus(m_handle.GetHandle(), &cBytes, &key,
                                           &ol, INFINITE);
...
class AsyncFileIO : public DiskPriorityIO {
 public:
  ...
  virtual bool Initialize(const char* filePath, int openMode,
    ...
    m_fileHandle.Reset(::CreateFileA(
    ...
    m_fileIocp.Reset(::CreateIoCompletionPort(m_fileHandle.GetHandle(), NULL,
                                              NULL, iocpThreads));
    for (int i = 0; i < iocpThreads; ++i) {
      m_fileIocpThreads.emplace_back(
          std::thread(std::bind(&AsyncFileIO::ListionIOCP, this)));
    }
  ...
  virtual std::uint64_t ReadBinary(std::uint64_t readSize, char* buffer,
    ...
    if (!::ReadFile(m_fileHandle.GetHandle(), buffer,
                    static_cast<DWORD>(readSize), nullptr, &col) &&
    ...
    BOOL ret = ::GetQueuedCompletionStatus(this->m_fileIocp.GetHandle(),
                                           &cBytes, &key, &ol, INFINITE);
    if (FALSE == ret || nullptr == ol) return 0;
    return readSize;
  }

  virtual std::uint64_t WriteBinary(std::uint64_t writeSize, const char* buffer,
                                    std::uint64_t offset = UINT64_MAX) {
    return 0;
  }
  ...
  virtual bool ReadFileAsync(AsyncReadRequest& readRequest) {
    ...
    if (!::ReadFile(m_fileHandle.GetHandle(), readRequest.m_buffer,
                    static_cast<DWORD>(readRequest.m_readSize), nullptr,
                    &col) &&
        GetLastError() != ERROR_IO_PENDING) {
 ...
 private:
  ...
  static uint64_t GetSectorSize(const char* p_filePath) {
    ...
    if (dwSize > 0) {
      ...
      if (lpBuffer) {
        DWORD dwResult =
            GetFullPathName(p_filePath, dwSize, lpBuffer, &lpFilePart);
        if (dwResult > 0 && dwResult <= dwSize) {
          ...
          if (lpBuffer[0] == _T('\\')) {
            ...
            if (((lpBuffer[0] >= _T('a') && lpBuffer[0] <= _T('z')) ||
                 (lpBuffer[0] >= _T('A') && lpBuffer[0] <= _T('Z'))) &&
                lpBuffer[1] == _T(':')) {
              if (lpBuffer[2] != 0) {
                lpBuffer[2] = _T('\\');
                lpBuffer[3] = 0;
...
class AsyncFileIO : public DiskPriorityIO {
 public:
  ...
  virtual bool Initialize(const char* filePath, int openMode,
    ...
    for (int i = 0; i < threadPoolSize; ++i) {
      m_fileIocpThreads.emplace_back(
          std::thread(std::bind(&AsyncFileIO::ListionIOCP, this, i)));
    }
#endif
    return true;
  }

  bool Initialize(int fileDescriptor, int openMode,
                  std::uint64_t maxIOSize = (1 << 20),
                  std::uint32_t maxReadRetries = 2,
                  std::uint32_t maxWriteRetries = 2,
                  std::uint16_t threadPoolSize = 4) {
    ...
    m_openByInit = false;
    if (m_fileHandle <= 0) {
      LOG(LogLevel::LL_Error, "Incorrect file handle: %d\n", fileDescriptor);
      return false;
    }
  ...
  virtual bool ReadFileAsync(AsyncReadRequest& readRequest) {
    ...
    while (curTry < maxTry &&
           syscall(__NR_io_submit,
                   m_iocps[(readRequest.m_status & 0xffff) % m_iocps.size()], 1,
                   iocbs) < 1) {
  ...
  virtual void ShutDown() {
    for (size_t i = 0; i < m_iocps.size(); i++)
      syscall(__NR_io_destroy, m_iocps[i]);
    if (m_openByInit) {
      close(m_fileHandle);
    }
#ifndef BATCH_READ
 ...
 private:
  ...
  void ListionIOCP(int i) {
    ...
    while (!m_shutdown) {
      int numEvents = syscall(__NR_io_getevents, m_iocps[i], b, b,
                              events.data(), &AIOTimeout);

      for (int r = 0; r < numEvents; r++) {
        AsyncReadRequest* req =
            reinterpret_cast<AsyncReadRequest*>((events[r].data));
        if (nullptr != req) {
          req->m_callback(req);
        }
      }
